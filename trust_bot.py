...
BUTTON_EMOJIS = {
    ...
    "paytm": "5334890573281114250",
    "bharatpe": "6215156189454409086",
...
async def handle_user_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("awaiting_transfer_amount"):
        ...
        await update.message.reply_text(f"✅ <b>Transfer Successful!</b>\n\nSent ₹{amt:.2f} to <code>{target_id}</code>\nYour new balance: ₹{sender_new:.2f}", parse_mode="HTML")
        try:
            await context.bot.send_message(chat_id=int(target_id), text=f"💰 <b>You received ₹{amt:.2f}!</b>\nFrom User ID: <code>{user_id}</code>", parse_mode="HTML")
        except:
            pass
        return
...
Paytm / BharatPe Payment Available
...
━━━━━━━━━━━━━━━━━━━━
            ...
            keyboard = [
                [
                    CB("BharatPe", style="primary", icon=get_button_emoji("bharatpe"), callback_data=f"gateway_2_{original_amount}"),
                    CB("Paytm", style="primary", icon=get_button_emoji("paytm"), callback_data=f"gateway_1_{original_amount}")
                ],
                [CB("Cancel", style="danger", icon=get_button_emoji("cancel"), callback_data="menu_add_balance")]
            ]
...
"""
    ...
    logger.info(f"Paytm Key (1)      : {KARANPAY_KEY_1}")
    logger.info(f"BharatPe Key (2)   : {KARANPAY_KEY_2}")
